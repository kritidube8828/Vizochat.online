...
// ---- Gift pop animation (rose/kiss/heart/diamond/crown/fireworks) ----
const GIFT_VISUALS = {
  3:   { emoji: '🌹', label: 'Rose' },
  29:  { emoji: '💋', label: 'Kiss' },
  69:  { emoji: '❤️', label: 'Heart' },
  149: { emoji: '💎', label: 'Diamond' },
  599: { emoji: '👑', label: 'Crown' },
  999: { emoji: '🎆', label: 'Fireworks' }
};

function showGiftAnimation(amount, fromMe) {
  const visual = GIFT_VISUALS[amount] || { emoji: '🎁', label: 'Gift' };
  ...
  burst.textContent = visual.emoji;
  ...
  label.textContent = fromMe ? `You sent a ${visual.label}! 🎉` : `${visual.label} received! 🎉`;
  overlay.appendChild(label);
  setTimeout(() => label.remove(), 1900);

  // Floating particles rising from the bottom for extra flourish
  const particleCount = amount >= 599 ? 18 : amount >= 69 ? 12 : 8;
  for (let i = 0; i < particleCount; i++) {
    setTimeout(() => {
      const p = document.createElement('div');
      p.className = 'gift-particle';
      p.textContent = visual.emoji;
